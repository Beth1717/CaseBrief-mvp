...
function record(action,target,details={},outcome='success',initiator=actor){workspace.events.push({id:crypto.randomUUID(),at:new Date().toISOString(),matterId:data.matter.id,sessionId,actor:{...initiator},action,target,outcome,details});persist()}
...
function srcChips(arr=[]){return arr.map(id=>{const d=data.documents.find(x=>x.id===id);return `<button class="pill source" onclick="event.stopPropagation();openDetail('document','${esc(id)}')">${esc(d?d.title:id)}</button>`}).join('')}
...
function nav(){
 const groups=[
  ...
  ['System',['umg','audit','safeguards']],
  ['Research',['landing','survey']]
 ];
 const labels={dashboard:'Case Overview',review:'Review Queue',timeline:'Court + Chronology',evidence:'Evidence',witnesses:'Witnesses',documents:'Documents',authorities:'Law & Authority',assistant:'AI Assistant',drafting:'Drafting Studio',communications:'Communications',umg:'UMG Runtime',audit:'Activity History',safeguards:'Safeguards',landing:'Public View',survey:'Attorney Survey'};
 document.getElementById('sidebar').innerHTML=groups.map(([g,items])=>`<div class="navgroup">${g}</div>${items.map(i=>`<button class="navbtn ${current===i?'active':''}" onclick="showView('${i}')">${labels[i]}</button>`).join('')}`).join('');
}
