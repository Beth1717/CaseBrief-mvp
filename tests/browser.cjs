...
(async()=>{
 ...
 await page.getByText(/events that led to State v. Jordan Hale began/).waitFor();
 ...
 await page.getByText(/Complete filing draft • attorney review required/).first().waitFor();
 await page.getByText('Motion to Suppress Evidence — Draft',{exact:true}).first().click();
